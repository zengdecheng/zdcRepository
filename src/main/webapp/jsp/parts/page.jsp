...
<s:if test="fsp.recordCount != 0">
	...
	<s:if test="${pageNo}>4">
 	...
 	<s:bean name="org.apache.struts2.util.Counter" id="counter">  
		...
		<s:iterator>  
			<s:if test="${pageNo} ==(current-1)"><span class="current">${current-1}</span></s:if>
			<s:else><a href="javascript:void(0)" onclick="turnOverPage(${current-1})">${current-1}</a></s:else>  
		</s:iterator>  
	</s:bean>
	<!-- 如果后面页数过多,显示... -->
	<s:if test="${end<(pageCount-2)}">&hellip;</s:if> 
	<s:if test="${end<(pageCount-1)}"><a href="javascript:void(0)" onclick="turnOverPage(${pageCount - 1})">${pageCount-1}</a></s:if> 
	<s:if test="${end<pageCount}"><a href="javascript:void(0)" onclick="turnOverPage(${pageCount})">${pageCount}</a></s:if> 
	<s:if test="${pageNo==pageCount}"><span class="disabled">下一页&nbsp;&raquo;</span></s:if> 
	<s:else><a href="javascript:void(0)" onclick="turnOverPage(${pageNo + 1})">下一页&nbsp;&raquo;</a></s:else>
	&nbsp;转到&nbsp;<select id="pagerSelect">
	<s:bean name="org.apache.struts2.util.Counter" id="counter">  
		<s:param name="first" value="1"/>  
		<s:param name="last" value="${pageCount}" />  
		<s:iterator>  
	   		<option value="${current-1}" <s:if test="${(current-1)==pageNo}">selected="selected"</s:if>>${current-1}</option>
		</s:iterator>  
	</s:bean>
	</select>&nbsp;页 
