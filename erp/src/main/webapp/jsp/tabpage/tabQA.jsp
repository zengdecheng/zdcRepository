...
<div id="iframeDiv">
	<form id="tabQA">
		<div style="background: #4b8df8; font-size: 14px; margin: 5px 5px 5px 0; color: #ffffff; width: 100%;">
			<p style="padding: 5px 5px">&nbsp;QA</p>
		</div>
		<div style="border-bottom: 1px solid rgb(205, 205, 205); font-size: 14px;">
			<p style="padding: 5px 5px 5px 10px; margin: 0;">车缝数量</p>
		</div>
		<div>
			<table id="sewing_num_show" class="yldp_table">
			</table>
		</div>
		<div style="border-bottom: 1px solid rgb(205, 205, 205); font-size: 14px;">
		...
		<div>
			<input type="hidden" name="oaQa.id" fillName="qaInfo.id"/>
			<input type="hidden" name="oaQa.oaOrderId" fillName="oaOrder.id"/>
			<input type="hidden" name="oaQa.qualifiedTotal" id="qualifiedTotal"/>
			<input type="hidden" name="oaQa.unqualifiedTotal" id="unqualifiedTotal"/>
			<input type="hidden" name="oaQa.qualifiedNumInfo" id="qualifiedNumInfo"/>
			<input type="hidden" name="oaQa.unqualifiedNumInfo" id="unqualifiedNumInfo"/>
			<table id="qualify_num_show" class="yldp_table">
			</table>
		</div>
		<div style="border-bottom: 1px solid rgb(205, 205, 205); font-size: 14px;">
			<p style="padding: 5px 5px 5px 10px; margin: 0;">检验次品数量</p>
		</div>
		<div>
			<table id="unqualify_num_show" class="yldp_table">
			</table>
		</div>
		<div style="border-bottom: 1px solid rgb(205, 205, 205); font-size: 14px;">
		...
		<div style="width: 96%; font-size: 14px; margin-left: 2.5%; padding: 5px 0 5px 0;">
			<table style="font-size: 12px; width: 100%;">
				<tr height="28px;">
					<td width="20%" id="operator">制单: <s:select theme="simple" list="@com.xbd.oa.utils.XbdBuffer@getStaffsByGroupName('qa')" listKey="map.login_name" listValue="map.login_name" cssClass="z_inp2 validate[required]" name="oaOrderDetail.worker" id="worker"/></td>
					<td width="45%">处理日期:<span elementFillName="oaOrderDetail.wf_real_start"></span></td>
					<td></td>
				</tr>
				<tr height="28px;">
						...
						<div style="height: 105px;">
							...
							<div style="height: 100px; width: 80%; float: right; border: 1px solid rgb(205, 205, 205);">
								...
								<ul style="margin: 0; padding: 0 0 0 10px; line-height: 19px;">
									...
									<li>流入颜色:
										<div id="detailRealStartColor" style="color:#fff;margin: 1px 0 4px; width:45px; text-align: center; display: inline-block;"></div>
									</li>
									<li>流出颜色:
										<div id="detailRealFinishColor" style="color:#fff;margin-bottom: 1px; width:45px; text-align: center; display: inline-block;"></div>
									</li>
				...
				<tr height="45px">
					<td colspan="3"><div style="float: left; line-height: 40px; margin-right: 15px;">附件：</div>
						<div class="attachment_name" id="otherFile" style="line-height: 40px; float: left; width: 50%; overflow: hidden; text-overflow: ellipsis;">&nbsp;</div> <input type="hidden" name="oaOrderDetail.otherFile" fillName="oaOrderDetail.other_file" id="hid_attachment_url">
		...
		<input type="hidden" id="orderId" name="orderId"> <input type="hidden" id="processOrder" name="processOrder" value="false"> <input type="hidden" id="wfStep" name="wfStep">
	</form>
	<div style="width: 98%; height: 40px; line-height: 40px; text-align: center;" id="processOrderDive">
		...
		<input type="button" value="退回上一步" class="process_button" id="backBtn" onclick="javascript:parent.backOrder('iframePurchase');">
...
<script type="text/javascript">
	$(function() {
		require([ "order/tabQA" ], function(fn) {
			fn.init();
		});
	});
</script>