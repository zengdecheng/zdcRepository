			...
			var biz = {
				...
				event : {
					getSellStaffs : function(e) {
						...
						$("#crm_staff_sales").append("<option value=''>请选择</option>");
						$.each(sellStaffJsons, function(i, n) {
							if (sales == sellStaffJsons[i].loginName) {
								$("#crm_staff_sales").append("<option selected value='" + sellStaffJsons[i].loginName + "'>" + sellStaffJsons[i].loginName + "</option>");
							} else {
								$("#crm_staff_sales").append("<option value='" + sellStaffJsons[i].loginName + "'>" + sellStaffJsons[i].loginName + "</option>");
							}
						});
					},
					// 从crm获取销售人员
					...
					getMrStaffs : function() {
						...
						$("#crm_staff_mr").empty();
						$("#crm_staff_mr").append("<option value=''>请选择</option>");
						$.each(mrStaffJsons, function(i, n) {
							if (mr_name == mrStaffJsons[i].loginName) {
								$("#crm_staff_mr").append("<option selected value='" + mrStaffJsons[i].loginName + "'>" + mrStaffJsons[i].loginName + "</option>");
							} else {
								$("#crm_staff_mr").append("<option value='" + mrStaffJsons[i].loginName + "'>" + mrStaffJsons[i].loginName + "</option>");
							}
						});
					},
					...
					resetFrom : function() {
						$(':input', '#orderList').not(':button, :submit, :reset, :hidden').val('').removeAttr('checked').removeAttr('selected');
					},
					outExcel : function() {
						$("#output").attr("disabled", "true");
						$("#output").val("30秒后再次点击");
